...
int maximiser_importance(void)
{
   ...
   return importance_max[nb_objets][poids_max];
}
...
int main(void)
{
   ...
   for(iObjet = nb_objets, iPoids = poids_max; iObjet > 0; --iObjet) {
      if(importance_max[iObjet][iPoids] > importance_max[iObjet - 1][iPoids]) {
         printf("%d ", iObjet);
