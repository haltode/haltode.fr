...
find ./pages -iname "*.md" -type f -exec sh -c \
   'pandoc --ascii "${0}" -o "./tmp/$(basename ${0%.md}.tmp.html)" && \
   cat ./templates/layout/header.html ./tmp/$(basename ${0%.md}.tmp.html) ./templates/layout/footer.html > ./templates/pages/$(basename ${0%.md}.html) && \
